...
router.get("/", async (_req: Request, res: Response) => {
    try {
        const result = await pg.query("SELECT * FROM requests ORDER BY created_at DESC");
...
router.post("/", async (req: Request, res: Response) => {
    try {
        ...
        const newRequest = await pg.query(
...
router.put("/:id", async (req: Request, res: Response) => {
    try {
        ...
        if (status === 'approved' && oldStatus !== 'approved') {
            ...
            const invCheck = await tx.query("SELECT units FROM inventory WHERE blood_group = $1", [blood_group]);
